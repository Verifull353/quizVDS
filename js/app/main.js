...
function generate_sections(title,idx){
  ...
  var section_html = '  <div class="col-lg-4">\
          <div class="portfolio-item argomento button_href" target="#test-seq">\
            <span class="caption">\
        ...
        loadBinaryFile('./js/quiz_VDS.sqlite', function(data){
          ...
          initSqlJs({ locateFile: filename => `./js/sqljs/${filename}` }).then(function(SQL){
            ...
            $.each(res[0].values,function(key,val){
              ...
              $('.button_href').on('click',function(){
                target = $(this).attr('target');
                $('.pushTOP').removeClass('pushTOP');
                $(target).toggleClass('pushTOP');
              })
